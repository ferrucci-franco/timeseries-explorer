...
function testCsvPreviewWarnsWhenPreambleHidesAllLoadedRows() {
    const csv = [
        '# instrument export',
        '# generated by lab',
        '# operator: test',
    ...
    const dialog = new CsvParsingPreviewDialog({ parser, sampleBuffer: buffer, csvProfile: profile, title: 'preamble-only-preview.csv' });
    dialog.preview = parser.inspectPreview(buffer, { maxRows: 4, delimiter: dialog.state.delimiter, encoding: dialog.state.encoding });
    dialog.state.hidePreambleRows = true;
    dialog._rebuildProfile();
    const message = dialog._hiddenRowsMessage(dialog._allPreviewRowEntries());
    assert(message.includes('hidden as preamble'), 'CSV preview should explain when all loaded rows are hidden as preamble');
}

function testCsvPreviewWarnsWhenOnlyPreambleIsLoaded() {
    const csv = [
        '# instrument export',
        '# generated by lab',
        '# operator: test',
    ...
    const dialog = new CsvParsingPreviewDialog({ parser, sampleBuffer: buffer, csvProfile: profile, title: 'preamble-only-preview.csv' });
    dialog.preview = parser.inspectPreview(buffer, { maxRows: 4, delimiter: dialog.state.delimiter, encoding: dialog.state.encoding });
    dialog.state.hidePreambleRows = false;
    dialog._rebuildProfile();
    const messageInfo = dialog._onlyPreambleLoadedMessageInfo(dialog._allPreviewRowEntries(), dialog._previewRowEntries());
    assert(messageInfo?.sourceIndex === 3, 'CSV preview should place the load-more message after the last loaded preamble row');
    assert(messageInfo.message.includes('Only preamble rows'), 'CSV preview should explain when Lines shown has not reached the data rows');
}
...
testCsvPreviewWarnsWhenPreambleHidesAllLoadedRows();
testCsvPreviewWarnsWhenOnlyPreambleIsLoaded();
