...
const releaseNotes = await readFile(new URL('docs/releases/v0.1.0-beta.6.md', root), 'utf8');

assert.equal(pkg.version, '0.1.0-beta.6');
assert.match(constants, /APP_VERSION = '0\.1\.0-beta\.6'/);
...
assert.match(pkg.scripts['desktop:dist:macos:x64'], /--mac dmg zip dir --x64/);
assert.match(pkg.scripts['desktop:dist:macos:arm64'], /--mac dmg zip dir --arm64/);
assert.match(pkg.scripts['desktop:dist:linux:x64'], /--linux deb AppImage dir --x64/);
...
assert.match(manifest.downloadUrl, /v0\.1\.0-beta\.6\/Time%20Series%20Explorer-0\.1\.0-beta\.6-setup-x64\.exe$/);
assert.match(manifest.portableUrl, /v0\.1\.0-beta\.6\/Time%20Series%20Explorer-0\.1\.0-beta\.6-portable-x64\.exe$/);
assert.equal(manifest.platforms.macos.status, 'available');
assert.deepEqual(
  manifest.platforms.macos.assets.map(asset => asset.fileName),
  [
    'Time Series Explorer-0.1.0-beta.6-mac-x64.dmg',
    'Time Series Explorer-0.1.0-beta.6-mac-x64.zip',
    'Time Series Explorer-0.1.0-beta.6-mac-arm64.dmg',
    'Time Series Explorer-0.1.0-beta.6-mac-arm64.zip'
  ]
);
...
assert.deepEqual(
  ...
  [
    'Time Series Explorer-0.1.0-beta.6-linux-amd64.deb',
    'Time Series Explorer-0.1.0-beta.6-linux-x86_64.AppImage'
  ]
);
