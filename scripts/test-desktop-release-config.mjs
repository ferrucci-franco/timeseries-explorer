...
const releaseNotes = await readFile(new URL('docs/releases/v0.1.0-beta.1.md', root), 'utf8');

assert.equal(pkg.version, '0.1.0-beta.1');
assert.match(constants, /APP_VERSION = '0\.1\.0-beta\.1'/);
...
assert.match(manifest.downloadUrl, /v0\.1\.0-beta\.1\/Time%20Series%20Explorer-0\.1\.0-beta\.1-setup-x64\.exe$/);
assert.match(manifest.portableUrl, /v0\.1\.0-beta\.1\/Time%20Series%20Explorer-0\.1\.0-beta\.1-portable-x64\.exe$/);
