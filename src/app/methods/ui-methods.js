...
proto.initEventListeners = function() {
    ...
    document.addEventListener('click', (e) => {
        ...
        if (!e.target.closest('#filter-gap-help-popover') && !e.target.closest('#filter-gap-help-toggle')) {
            this._toggleFilterGapHelpPopover?.(false);
        }
        if (!e.target.closest('#filter-direction-help-popover') && !e.target.closest('#filter-direction-help-toggle')) {
            this._toggleFilterDirectionHelpPopover?.(false);
        }
        if (!e.target.closest('#filter-design-help-popover') && !e.target.closest('#filter-design-help-toggle')) {
            this._toggleFilterDesignHelpPopover?.(false);
        }
        if (!e.target.closest('#xcorr-help-popover') && !e.target.closest('#xcorr-help-toggle')) {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Escape' && !document.getElementById('filter-init-help-popover')?.hidden) {
            e.preventDefault();
            this._toggleFilterInitHelpPopover?.(false);
            return;
        }
        if (e.key === 'Escape' && !document.getElementById('filter-gap-help-popover')?.hidden) {
            e.preventDefault();
            this._toggleFilterGapHelpPopover?.(false);
            return;
        }
        if (e.key === 'Escape' && !document.getElementById('filter-direction-help-popover')?.hidden) {
            e.preventDefault();
            this._toggleFilterDirectionHelpPopover?.(false);
            return;
        }
        if (e.key === 'Escape' && !document.getElementById('filter-design-help-popover')?.hidden) {
            e.preventDefault();
            this._toggleFilterDesignHelpPopover?.(false);
            return;
        }
        if (e.key === 'Escape' && !document.getElementById('xcorr-help-popover')?.hidden) {
            e.preventDefault();
            this._toggleXcorrHelpPopover?.(false);
            return;
        }
