...
function isAutoGeneratedHeaderlessProfile(profile = {}) {
    if (profile?.headerlessLocked === true) return true;
    return profile?.hasHeader === false
        && hasGeneratedColumnHeaders(profile?.rawHeaders)
        && profile?.profileSource !== 'user';
}
...
export default class CsvParsingPreviewDialog {
    ...
    constructor({ parser, sampleBuffer, sampleSegments = null, loadPreviewSegment = null, csvProfile = null, title = '' } = {}) {
        ...
        this.headerlessProfileLocked = isAutoGeneratedHeaderlessProfile(this.autoProfile);
        this.state = {
            lineLimit: 10,
    ...
    _profileUsesInlineUnits(profile = {}) {
        ...
        if (rawHeaders.length < 1 || !headers.length) return false;
    ...
    _rebuildProfile() {
        ...
        const usesGeneratedIndex = this.state.timeMode === 'index';
        if (rawHeaders.length < 1) {
            messages.push('Header/data row must expose at least one column.');
        } else if (!usesGeneratedIndex && rawHeaders.length < 2) {
            messages.push('Header/data row must expose at least two columns.');
        }
        ...
        this.resultProfile = {
            ...
            headerlessLocked: this.headerlessProfileLocked ? true : false,
            profileSource: 'user',
