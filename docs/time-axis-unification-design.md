...
`_timeAxisModel` resolver (applies `interpretationOverride`); legacy→canonical adapter (§6); **migrate every reader** — `_timeKind`, `_timeDisplayMode`, `_fftTimeKind` (`fft-methods.js:1059`), and direct `metadata.timeKind` reads in data-tools & live-update — onto the resolver or thin wrappers (do not rely on grep to find them: two files are NUL-invisible, §12.1); extend **both** normalizers (`data-methods.js:99-133` **(v)**, `file-methods.js:2499-2533` **(v)**); session bump + migrator (loader rejects mismatched version by exact equality, `session-methods.js:312-314` **(v)**) incl. tagged crop/shift. *Acceptance:* identical displays/titles/signatures; **two generic-numeric files still overlay (C1 non-regression)**; v1 sessions migrate; golden snapshots unchanged.
